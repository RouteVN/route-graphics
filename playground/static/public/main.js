...
const recursivelyLoadAssets = (objects) => {
  ...
  const processObject = (obj) => {
    ...
    if (obj?.type !== "sound") {
      ...
      if (
        obj.soundSrc &&
        typeof obj.soundSrc === "string" &&
        !seenAssets.has(obj.soundSrc)
      ) {
...
const initRouteGraphics = async () => {
  try {
    ...
    errorMsg.textContent =
      "Failed to initialize Route-Graphics: " + error.message;
...
const updateAndHighlight = (highlightedInput, input) => {
  ...
  highlightedInput.innerHTML = text
